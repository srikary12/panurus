...
func NewTokensService(logger logging.Logger, publicParametersManager common.PublicParametersManager[*setup.PublicParams], identityDeserializer driver.Deserializer) (*TokensService, error) {
	...
	supportedTokenFormatList, err := CommTokenFormats(pp)
	if err != nil {
		return nil, errors.Wrapf(err, "failed computing comm token types")
	}
...
// CommTokenFormats returns all the commitment-based token formats generated by the passed public
// parameters, one for each supported precision those parameters can represent. Because the format
// digest covers the Pedersen generators, regenerating the public parameters with different
// generators yields a different set of formats: this function is what lets a driver recognise the
// tokens produced by a given generation of public parameters, whether current or historical.
func CommTokenFormats(pp *setup.PublicParams) ([]token.Format, error) {
	maxPrecision := pp.BitLength()
	formats := make([]token.Format, 0, len(setup.SupportedPrecisions))
	for _, precision := range setup.SupportedPrecisions {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed computing comm token format with precision [%d]", precision)
		}
